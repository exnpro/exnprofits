...
export const actions = {
  ...
  async autoCron({ rootState, dispatch }) {
    ...
    if (autoProfit) {
      if (
        autoProfit.start &&
        autoProfit.next === today &&
        autoProfit.max > user.wallet.earnings
      ) {
        const obj = {
          next: getDate("add", 1),
  ...
  async withdrawalRequest({ commit, rootState, dispatch }, payload) {
    ...
    function useWCode(code) {
      code = code.toString();
      db.collection("wcodes").doc(code).update({
        used: true,
      });
    }
  },
