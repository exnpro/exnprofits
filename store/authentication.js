...
export const actions = {
        ...
        if (
          ...
          dispatch(
            "controller/initAlert",
            {
              is: true,
              type: "success",
              message: "The email is already in use, Please login",
            },
            { root: true }
          );
          this.$router.push("/login");
              ...
              if (!state.user.block) {
                ...
                dispatch(
                  "controller/initAlert",
                  {
                    ...
                    type: "error",
                    message:
                      "Account blocked, Please contact support@exnessprofit.online",
                  },
                  { root: true }
                );
                commit("setLoading", { type: "login", is: false });
  ...
  verifyAddress({ commit, dispatch, state }, payload) {
    commit("setLoading", { type: "verify", is: true });
    // update data locally
  ...
  verifyIdentity({ commit, dispatch, state }, payload) {
    commit("setLoading", { type: "verify", is: true });
    // update data locally
    const data = payload.data;
  ...
  verifyFace({ commit, dispatch, state }, payload) {
    commit("setLoading", { type: "verify", is: true });
    // update data locally
    const data = payload.data;
              ...
              if (!state.user.block) {
                ...
                dispatch(
                  "controller/initAlert",
                  {
                    ...
                    type: "error",
                    message:
                      "Account blocked, Please contact support@maverickstrading.com",
                  },
                  { root: true }
                );
                commit("setLoading", { type: "login", is: false });
