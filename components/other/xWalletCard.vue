    ...
    <v-card-text class="d-flex flex-column py-2">
      ...
      <div style="width: 100%" class="ml-2 my-1">
        ...
        <div class="text-h6 text-sm-h5 white--text font-weight-medium">
          ...
          <span v-if="type === 'number'"> {{ number.toLocaleString() }}</span>
        </div>
        <div v-if="type === 'amount'" class="text-caption d-flex alig-center">
          <v-icon size="20" fabs color="white" class="mr-1">
            mdi-bitcoin
          </v-icon>
          {{ amount | btc }} <span class="font-weight-medium">BTC</span>
        </div>
        <div
          v-if="type === 'text'"
          class="text-caption white--text text-sm-subtitle-2 d-flex alig-center"
        >
          {{ info }}
        </div>
      </div>
