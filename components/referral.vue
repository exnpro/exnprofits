<template>
  <p-container dark :color="'primary lighten-1'" title="Affiliate Program">
    <template v-slot:header>
      <v-spacer />
      Earnings
      <v-chip outlined label color="secondary" class="ml-2">
        <span v-if="user && user.wallet.earnings !== 0"
          >{{ currency }}{{ (user && user.wallet.earnings) | currency }}</span
        >
    ...
    <v-row class="ma-0">
      ...
      >
        ...
        <v-btn
          ...
          @click="
            copy(
              `https://exnessprofit.com/register/${user && user.referralID}`
            )
          "
        >
          <v-icon left small> mdi-content-copy </v-icon>
          <span style="width: 50px" class="text-truncate"
            >https://exnessprofit/register/{{
