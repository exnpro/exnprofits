  ...
  >
    ...
    <v-row class="ma-0">
      ...
      >
        ...
        <v-spacer />
        <div
          v-if="user && user.wallet.deposit !== 0"
          class="white--text font-weight-medium"
        >
          {{ currency }}{{ (user && user.wallet.deposit) | currency }}
        </div>
        <div v-else class="white--text font-weight-medium">$0.00</div>
      </v-col>
      <v-col
        cols="12"
        class="d-flex rounded text-subtitle-2 font-weight-medium"
      >
        <span>Min Amount</span>
        <v-spacer />
        <div class="font-weight-medium">
          {{ currency }}{{ (plan.range && plan.range.from) | currency }}
        </div>
      </v-col>
      <v-col
        cols="12"
        class="py-0 d-flex rounded text-subtitle-2 font-weight-medium"
      >
        <span>Max Amount</span>
        <v-spacer />
        <div class="font-weight-medium">
          {{ currency }}{{ (plan.range && plan.range.to) | currency }}
        </div>
      </v-col>
    </v-row>
  ...
  methods: {
    ...
    submit() {
      ...
      if (this.checkBalance(amount)) {
        if (amount < this.plan.range.from) {
          this.initAlert({
            is: true,
            type: "error",
            persistence: true,
            message: `Amount must be greater than ${this.currency}${this.plan.range.from}`,
          });
          return false;
        } else {
          const payload = {
