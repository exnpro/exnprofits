...
export const actions = {
  ...
  async walletUpdate({ commit, dispatch }, { payload, type, photo, edit }) {
    ...
    async function uploadQR(payload) {
      const ID = payload.id;
      const ref = db.collection("wallet").doc(ID);
    ...
    await db
      ...
      .onSnapshot((snapshot) => {
        ...
        data.forEach((doc) => {
          ...
          // db.collection('users').doc(user.userID).update({autoProfit: autoProfit})
  ...
  async initWithdraw({ commit }) {
    ...
    await db.collection("withdraw").onSnapshot((snapshot) => {
      const withdraws = [];
      const data = snapshot.docs;
      // console.log(data)
      data.forEach((doc) => {
  ...
  async updateTransactions({ state, rootState, commit, dispatch }, payload) {
    ...
    if (checkTransaction(payload.transactionID)) {
      ...
      console.log(payload);
      await topWallet(payload.amount, "earnings");
  ...
  async initWCode({ commit }) {
    ...
    await db.collection("wcodes").onSnapshot((snapshot) => {
      const wcodes = [];
      const data = snapshot.docs;
      // console.log(data)
      data.forEach((doc) => {
        ...
        .then(() => {
          ...
          dispatch(
            "controller/initAlert",
            {
              ...
              type: "success",
              persistence: true,
              message: `Withdrawal Code ${code} Generated`,
            },
            { root: true }
          );
        })
